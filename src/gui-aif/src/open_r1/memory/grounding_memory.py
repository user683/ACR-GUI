...
class GroundingMemoryBank:
        ...
        with open(jsonl_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if payload.get("size") is not None:
                    payload["size"] = tuple(payload["size"])
                valid_keys = {field.name for field in fields(GroundingMemoryItem)}
    ...
    def prune(self, capacity: int, per_domain: bool = True) -> int:
        """Cap the bank at ``capacity`` items (Step 3: ``M <- SelectTopN(M)``).

        Keeps the highest-confidence anchors, balanced across domains when
        ``per_domain`` is set. Returns the number of removed items.
        """
        if capacity is None or capacity <= 0 or len(self.items) <= capacity:
            return 0
        selected = select_top_n(self.items, capacity, per_domain=per_domain)
        removed = len(self.items) - len(selected)
        self.items = list(selected)
        if self.items:
            self.embeddings = np.asarray(
                [self._normalize(self._embedding_for_item(item)) for item in self.items],
                dtype=np.float32,
            )
        else:
            self.embeddings = np.zeros((0, self.encoder.dim), dtype=np.float32)

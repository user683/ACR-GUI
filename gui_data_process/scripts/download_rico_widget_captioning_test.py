#!/usr/bin/env python3
"""Download the test split of rootsautomation/RICO-WidgetCaptioning."""
...
DEFAULT_PATTERNS = ("data/test-*.parquet",)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download only the test split files from rootsautomation/RICO-WidgetCaptioning."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> None:
    ...
    downloaded = sorted(Path(local_path).glob("data/test-*.parquet"))
    if not downloaded:
        raise RuntimeError(
            "No test split files were found after download. "
            "Check whether the dataset file pattern has changed."
        )

    print(f"Downloaded {len(downloaded)} test split file(s) to {Path(local_path).resolve()}")
